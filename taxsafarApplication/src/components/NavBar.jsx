  ...
  return (
    <header className="sticky top-0 z-20 border-b border-slate-200/70 bg-amber-50/85 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <Link className="group flex items-center gap-3 text-slate-900" to="/" onClick={handleLogoClick}>
        ...
        <nav className="order-3 flex w-full flex-wrap items-center justify-center gap-5 text-sm font-semibold text-slate-600 lg:order-2 lg:w-auto">
          ...
          {navItems.map((item) => {
            if (item.label === 'Process') {
              return (
                <button
                  className="nav-link-fancy relative transition duration-300 hover:-translate-y-0.5 hover:text-slate-900 bg-none border-none cursor-pointer p-0"
                  key={item.label}
              ...
              return (
                <Link
                  className="nav-link-fancy relative transition duration-300 hover:-translate-y-0.5 hover:text-slate-900"
                  key={item.label}
                  to={item.href}
                >
                  {item.label}
